    ...
    const int startnumber = 0)
{
  ...
  // se->Verbosity(10);
  gSystem->mkdir(gSystem->DirName(out_root.c_str()), kTRUE);
  ...
  // se->run(nEvents);
  int nproc = se->run(nEvents);
  std::cout << "[MINIDEBUG] processed=" << nproc << std::endl;
