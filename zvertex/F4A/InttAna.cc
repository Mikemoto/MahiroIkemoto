...
int InttAna::Init(PHCompositeNode * /*topNode*/)
{
  ...
  h_ntp_clus = new TNtuple("ntp_clus", "Cluster Ntuple", "nclus:nclus2:bco_full:evt:size:adc:x:y:z:lay:lad:sen:lx:ly:phisize:zsize:zv:truthzv");
...
vector<int> InttAna::GetFlagVector(uint64_t arg)
{
  vector<int> flag_vector(64);
  for (int i = 0; i < 64; i++)
  {
    int flag = (arg >> i) & 1; // 1: true, 0: false
    flag_vector[i] = flag;
  }
...
int InttAna::process_event(PHCompositeNode *topNode)
{
  ...
  */
  ...
  double mbdz = (mbdout != nullptr) ? mbdout->get_zvtx() : -9999; // float?

  cout << "MBD zvtx directly(tabun...) : " << mbdout->get_zvtx() << endl;
  cout << "MBD zvtx(tabun...) : " << mbdz << endl;

  Gl1RawHit *gl1raw = findNode::getClass<Gl1RawHit>(topNode, "GL1RAWHIT");
  if (!gl1raw)
  {
    cout << "No Gl1Raw"
         << "qwertyuioplkjhgfdsazxcvbnjuytfrdswertghjk"
         << endl;
  }

  Gl1Packet *gl1packet = findNode::getClass<Gl1Packetv2>(topNode, "GL1RAWHIT");
  if (!gl1packet)
  {
    cout << "No Gl1packet" << endl
         << endl
         << endl;
  }

  // truth
  double zvertex_truth = -9999;
  if (m_useSimVtx)
  {
    auto truth_node = findNode::getClass<PHG4TruthInfoContainer>(topNode, "G4TruthInfo");
    const auto main_vertex_id = truth_node->GetPrimaryVertexIndex();
    const auto vertex_truth_temp = truth_node->GetPrimaryVtx(main_vertex_id);
    cout << "("
         //  << setw(8) << setprecision(4) << vertex_truth.X() << ", "
         //  << setw(8) << setprecision(4) << vertex_truth.Y() << ", "
         << setw(8) << setprecision(4) << vertex_truth_temp->get_z() << ") "
         << endl;
    // hist_->Fill( vertex_truth.Z() );
    zvertex_truth = vertex_truth_temp->get_z();
  }

  // auto vertex_truth = ROOT::Math::XYZTVector(vertex_truth_temp->get_x(), vertex_truth_temp->get_y(), vertex_truth_temp->get_z(), vertex_truth_temp->get_t());

  if (gl1packet != nullptr)
  {
    // ////////////////////////
    // // Gl1Packetv2
    // ////////////////////////
    // event_id_ = gl1_packet->getEvtSequence();
    // bunch_num_ = gl1_packet->getBunchNumber();
    // //  uint64_t trigger_input = gl1_packet->getTriggerInput();

    uint64_t trigger_vector_value = gl1packet->getTriggerVector();
    auto trigger_vector_ = this->GetFlagVector(trigger_vector_value);

    if (trigger_vector_[14] == false)
    {
      cout << "mbd trigger naiyo-" << endl;
      return Fun4AllReturnCodes::ABORTEVENT;
    }
  }
  ...
  else
  {
    ...
    for (clusterIter = clusterEnd.first; clusterIter != clusterEnd.second; clusterIter++)
    {
      ...
      if (recoCluster->get_ecore() > 0.15)
        nemc1++;
      if (recoCluster->get_ecore() > 0.15 && recoCluster->getNTowers() > 1)
        nemc2++;
    }
  }
  ...
  for (unsigned int inttlayer = 0; inttlayer < 4; inttlayer++)
  {
    ...
    for (const auto &hitsetkey : m_clusterMap->getHitSetKeys(TrkrDefs::TrkrId::inttId, inttlayer + 3))
    {
      ...
      for (auto clusIter = range.first; clusIter != range.second; ++clusIter)
      {
        ...
        ntpval[17] = zvertex_truth; // truth zvtx;
  ...
  */
  ...
  // if(nCluster<500)
  {
    ...
    if (clusterContainer != nullptr)
    {
      ...
      for (vector<cluspair>::iterator itrcp = vcluspair.begin(); itrcp != vcluspair.end(); ++itrcp)
      {
        ...
        for (clusterIter = clusterEnd.first; clusterIter != clusterEnd.second; clusterIter++)
        {
          ...
          if (dphithe < small_dphithe)
          {
            small_dphithe = dphithe;
            closeCluster = recoCluster;
          }
          if (dphi_abs < small_dphi)
          {
            small_dphi = dphi_abs;
            closeCluster = recoCluster;
          }
          if (dthe_abs < small_dthe)
          {
            small_dthe = dthe_abs;
          }
        }
    ...
    for (vector<cluspair>::iterator itrcp = vcluspair.begin(); itrcp != vcluspair.end(); ++itrcp)
    {
      cluspair &clspair = *itrcp;
      if (clspair.charge > 0)
        vcpairp.push_back(clspair);
      else
        vcpairm.push_back(clspair);
    }
    ...
    float &thev, float &ptep, float &ptem, float &phiv)
{
  ...
  if (fabs(wl - 1.0) > 0.00001)
    cout << "Calculation error in W vector" << endl;
